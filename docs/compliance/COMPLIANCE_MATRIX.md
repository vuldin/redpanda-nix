...
## Appendix B: Layered Compliance Architecture

**Key Question**: "Will the Redpanda Nix package remain compliant regardless of underlying OS?"
...
### OS-Independent Compliance Features
...
- **Anduril STIG**: 40% app-level (OS-independent), 60% requires OS baseline
- **FedRAMP High**: 40% app-level (OS-independent), 60% requires OS baseline

For full STIG/FedRAMP compliance, combine this package with an OS-specific security baseline.

---

## Appendix C: ROI & Competitive Analysis
...
**Break-Even**: Single federal cloud contract ($1-10M annually) covers the $430K FedRAMP investment.

### Nix-Based vs Traditional Deployment

| Aspect | Traditional (yum/apt) | Nix-Based |
|--------|----------------------|-----------|
| Reproducible builds | No | Yes (architectural) |
| Audit trail | syslog (limited) | Git (complete) |
| Rollback | Manual | Atomic (< 1 min) |
| FIPS compliance | Partial (container limitations) | System-wide enforcement |
| Evidence collection | Custom scripts | Single script, all frameworks |
| Annual compliance overhead | $50-100K | $10-20K |
...
# Git audit trail (SOC 2 CC8.1, NIST 800-161 SR-3, ISO 27036 7.2, FedRAMP CM-3)
git log --all --format="%H|%an|%ae|%ad|%s" > evidence/git-audit.csv

# Store integrity (SOC 2 CC7.2, NIST 800-161 SR-10, FedRAMP SI-7)
nix-store --verify --check-contents > evidence/store-integrity.txt

# SBOM (NIST 800-161, ISO 27036 7.3, FedRAMP SA-4)
sbomnix $(nix-build default.nix) --sbom cyclonedx --output evidence/redpanda-sbom.json

# Access controls (SOC 2 CC6.1, ISO 27036 6.9, FedRAMP AC-6)
systemctl show redpanda > evidence/systemd-hardening.txt

# Logs (SOC 2 CC7.1, ISO 27036 6.13, FedRAMP AU-3)
journalctl -u redpanda --since "30 days ago" -o json > evidence/logs.json
```

---

**Document Version**: 3.0 (Consolidated from SOC2_COMPLIANCE.md, COMPLIANCE_ARCHITECTURE.md, COMPLIANCE_SUMMARY.md, COMPLIANCE_COMPARISON.md, and COMPLIANCE_GAP_ANALYSIS.md)
